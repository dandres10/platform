...
async def save(params: RolSave, config: Config = Depends(get_config)) -> Response:
    return rol_controller.save(config=config, params=params)
...
async def update(
    ...
    return rol_controller.update(config=config, params=params)
...
async def list(params: Pagination, config: Config = Depends(get_config)) -> Response:
    return rol_controller.list(config=config, params=params)
...
async def delete(id: UUID4, config: Config = Depends(get_config)) -> Response:
    ...
    return rol_controller.delete(config=config, params=build_params)
...
async def read(id: UUID4, config: Config = Depends(get_config)) -> Response:
    ...
    return rol_controller.read(config=config, params=build_params)
