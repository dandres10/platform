...
async def save(params: CompanySave, config: Config = Depends(get_config)) -> Response:
    return await company_controller.save(config=config, params=params)
...
async def update(
    ...
    return await company_controller.update(config=config, params=params)
...
async def list(params: Pagination, config: Config = Depends(get_config)) -> Response:
    return await company_controller.list(config=config, params=params)
...
async def delete(id: UUID4, config: Config = Depends(get_config)) -> Response:
    ...
    return await company_controller.delete(config=config, params=build_params)
...
async def read(id: UUID4, config: Config = Depends(get_config)) -> Response:
    ...
    return await company_controller.read(config=config, params=build_params)
