...
class TranslationController:
    ...
    def save(self, config: Config, params: TranslationSave) -> Response:
        result_save = self.translation_save_use_case.execute(config=config, params=params)
    ...
    def update(self, config: Config, params: TranslationUpdate) -> Response:
        result_update = self.translation_update_use_case.execute(config=config, params=params)
    ...
    def list(self, config: Config, params: Pagination) -> Response:
        result_list = self.translation_list_use_case.execute(config=config, params=params)
    ...
    def delete(self, config: Config, params: TranslationDelete) -> Response:
        result_delete = self.translation_delete_use_case.execute(config=config, params=params)
    ...
    def read(self, config: Config, params: TranslationRead) -> Response:
        result_delete = self.translation_read_use_case.execute(config=config, params=params)
        if isinstance(result_delete, str):
            return Response.error(None, result_delete)
        return Response.success_temporary_message(
            response=result_delete,
