...
from src.infrastructure.database.repositories.location_repository import (
    LocationRepository,
)
...
class LocationController:
    ...
    def save(self, config: Config, params: LocationSave) -> Response:
        result_save = self.location_save_use_case.execute(config=config, params=params)
    ...
    @execute_transaction(layer=LAYER.I_W_C_E.value, enabled=settings.has_track)
    def update(self, config: Config, params: LocationUpdate) -> Response:
        result_update = self.location_update_use_case.execute(config=config, params=params)
    ...
    def list(self, config: Config, params: Pagination) -> Response:
        result_list = self.location_list_use_case.execute(config=config, params=params)
    ...
    @execute_transaction(layer=LAYER.I_W_C_E.value, enabled=settings.has_track)
    def delete(self, config: Config, params: LocationDelete) -> Response:
        result_delete = self.location_delete_use_case.execute(config=config, params=params)
    ...
    @execute_transaction(layer=LAYER.I_W_C_E.value, enabled=settings.has_track)
    def read(self, config: Config, params: LocationRead) -> Response:
        result_delete = self.location_read_use_case.execute(config=config, params=params)
        if isinstance(result_delete, str):
            return Response.error(None, result_delete)
        return Response.success_temporary_message(
            response=result_delete,
