...
from src.infrastructure.database.repositories.entities.rol_permission_repository import (
...
class RolPermissionController:
    ...
    async def save(self, config: Config, params: RolPermissionSave) -> Response:
        result_save = await self.rol_permission_save_use_case.execute(config=config, params=params)
    ...
    @execute_transaction(layer=LAYER.I_W_C_E.value, enabled=settings.has_track)
    async def update(self, config: Config, params: RolPermissionUpdate) -> Response:
        result_update = await self.rol_permission_update_use_case.execute(config=config, params=params)
    ...
    async def list(self, config: Config, params: Pagination) -> Response:
        result_list = await self.rol_permission_list_use_case.execute(config=config, params=params)
    ...
    @execute_transaction(layer=LAYER.I_W_C_E.value, enabled=settings.has_track)
    async def delete(self, config: Config, params: RolPermissionDelete) -> Response:
        result_delete = await self.rol_permission_delete_use_case.execute(config=config, params=params)
    ...
    @execute_transaction(layer=LAYER.I_W_C_E.value, enabled=settings.has_track)
    async def read(self, config: Config, params: RolPermissionRead) -> Response:
        result_delete = await self.rol_permission_read_use_case.execute(config=config, params=params)
        if isinstance(result_delete, str):
            return Response.error(None, result_delete)
        return Response.success_temporary_message(
            response=result_delete,
