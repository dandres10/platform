...
class UserLocationRolController:
    ...
    async def save(self, config: Config, params: UserLocationRolSave) -> Response:
        result_save = await self.user_location_rol_save_use_case.execute(config=config, params=params)
    ...
    @execute_transaction(layer=LAYER.I_W_C_E.value, enabled=settings.has_track)
    async def update(self, config: Config, params: UserLocationRolUpdate) -> Response:
        result_update = await self.user_location_rol_update_use_case.execute(config=config, params=params)
    ...
    async def list(self, config: Config, params: Pagination) -> Response:
        result_list = await self.user_location_rol_list_use_case.execute(config=config, params=params)
    ...
    @execute_transaction(layer=LAYER.I_W_C_E.value, enabled=settings.has_track)
    async def delete(self, config: Config, params: UserLocationRolDelete) -> Response:
        result_delete = await self.user_location_rol_delete_use_case.execute(config=config, params=params)
    ...
    @execute_transaction(layer=LAYER.I_W_C_E.value, enabled=settings.has_track)
    async def read(self, config: Config, params: UserLocationRolRead) -> Response:
        result_delete = await self.user_location_rol_read_use_case.execute(config=config, params=params)
        if isinstance(result_delete, str):
            return Response.error(None, result_delete)
        return Response.success_temporary_message(
            response=result_delete,
