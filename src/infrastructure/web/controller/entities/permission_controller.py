...
class PermissionController:
    ...
    async def save(self, config: Config, params: PermissionSave) -> Response:
        result_save = await self.permission_save_use_case.execute(config=config, params=params)
    ...
    @execute_transaction(layer=LAYER.I_W_C_E.value, enabled=settings.has_track)
    async def update(self, config: Config, params: PermissionUpdate) -> Response:
        result_update = await self.permission_update_use_case.execute(config=config, params=params)
    ...
    async def list(self, config: Config, params: Pagination) -> Response:
        result_list = await self.permission_list_use_case.execute(config=config, params=params)
    ...
    @execute_transaction(layer=LAYER.I_W_C_E.value, enabled=settings.has_track)
    async def delete(self, config: Config, params: PermissionDelete) -> Response:
        result_delete = await self.permission_delete_use_case.execute(config=config, params=params)
    ...
    @execute_transaction(layer=LAYER.I_W_C_E.value, enabled=settings.has_track)
    async def read(self, config: Config, params: PermissionRead) -> Response:
        result_delete = await self.permission_read_use_case.execute(config=config, params=params)
        if isinstance(result_delete, str):
            return Response.error(None, result_delete)
        return Response.success_temporary_message(
            response=result_delete,
