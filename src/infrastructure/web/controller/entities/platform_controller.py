...
class PlatformController:
    ...
    async def save(self, config: Config, params: PlatformSave) -> Response:
        result_save = await self.platform_save_use_case.execute(config=config, params=params)
    ...
    @execute_transaction(layer=LAYER.I_W_C_E.value, enabled=settings.has_track)
    async def update(self, config: Config, params: PlatformUpdate) -> Response:
        result_update = await self.platform_update_use_case.execute(config=config, params=params)
    ...
    async def list(self, config: Config, params: Pagination) -> Response:
        result_list = await self.platform_list_use_case.execute(config=config, params=params)
    ...
    @execute_transaction(layer=LAYER.I_W_C_E.value, enabled=settings.has_track)
    async def delete(self, config: Config, params: PlatformDelete) -> Response:
        result_delete = await self.platform_delete_use_case.execute(config=config, params=params)
    ...
    @execute_transaction(layer=LAYER.I_W_C_E.value, enabled=settings.has_track)
    async def read(self, config: Config, params: PlatformRead) -> Response:
        result_delete = await self.platform_read_use_case.execute(config=config, params=params)
        if isinstance(result_delete, str):
            return Response.error(None, result_delete)
        return Response.success_temporary_message(
            response=result_delete,
