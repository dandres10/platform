...
from sqlalchemy import Column, String, Boolean, DateTime, text, Text, Integer, Float, Time, Numeric
...
class LocationEntity(Base):
    ...
    country_id = Column(UUID(as_uuid=True), nullable=True)  # FK a geo_division(id) - nodo tipo COUNTRY
    city_id = Column(UUID(as_uuid=True), nullable=True)  # FK a geo_division(id) - nodo tipo CITY
    name = Column(String(255), nullable=False)
    address = Column(Text, nullable=False)
    phone = Column(String(20), nullable=False)
    email = Column(String(100), nullable=False)
    main_location = Column(Boolean, nullable=False, server_default=text('false'))
    latitude = Column(Numeric(10, 8), nullable=True)  # -90 a 90
    longitude = Column(Numeric(11, 8), nullable=True)  # -180 a 180
