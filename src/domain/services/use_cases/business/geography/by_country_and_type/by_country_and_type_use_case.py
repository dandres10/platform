...
class ByCountryAndTypeUseCase:
    ...
    async def execute(
        ...
        result = await self.geography_repository.get_by_country_and_type(
            config=config, country_id=params.country_id, type_name=params.type_name
        )
        if not result:
