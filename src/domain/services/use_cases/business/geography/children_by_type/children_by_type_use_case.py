...
class ChildrenByTypeUseCase:
    ...
    async def execute(
        ...
        result = await self.geography_repository.get_children_by_type(
            config=config, parent_id=params.parent_id, type_name=params.type_name
        )
        if not result:
