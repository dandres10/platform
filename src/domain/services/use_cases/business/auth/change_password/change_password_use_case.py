...
class ChangePasswordUseCase:
    ...
    async def execute(
        ...
        user = await self.user_read_uc.execute(
            config=config, params=UserRead(id=user_id)
        )
        if isinstance(user, str) or user is None:
            return await self.message.get_message(
                config=config,
                message=MessageCoreEntity(
                    key=KEYS_MESSAGES.AUTH_CHANGE_PASSWORD_USER_NOT_FOUND.value
                ),
            )

        if not self.password.check_password(
            password=params.old_password, hashed_password=user.password
        ):
            return await self.message.get_message(
                config=config,
                message=MessageCoreEntity(
                    key=KEYS_MESSAGES.AUTH_CHANGE_PASSWORD_OLD_INCORRECT.value
                ),
            )
