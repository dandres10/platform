...
from src.domain.models.entities.platform.index import PlatformDelete, Platform
from src.domain.services.repositories.entities.i_platform_repository import (
    IPlatformRepository,
)


class PlatformDeleteUseCase:
    ...
    async def execute(
        ...
        result = await self.platform_repository.delete(config=config, params=params)
        if not result:
