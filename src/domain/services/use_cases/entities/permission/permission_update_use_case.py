...
from src.domain.models.entities.permission.index import Permission, PermissionUpdate
...
class PermissionUpdateUseCase:
    ...
    async def execute(
        ...
        result = await self.permission_repository.update(config=config, params=params)
        if not result:
