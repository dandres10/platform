...
from src.domain.models.entities.permission.index import Permission
...
class PermissionListUseCase:
    ...
    async def execute(
        ...
        config: Config,
        params: Pagination,
    ) -> Union[List[Permission], str, None]:
        results = await self.permission_repository.list(config=config, params=params)
        if not results:
