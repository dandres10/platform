...
from src.domain.models.entities.permission.index import Permission, PermissionRead
from src.domain.services.repositories.entities.i_permission_repository import (
    IPermissionRepository,
)


class PermissionReadUseCase:
    ...
    async def execute(
        ...
        result = await self.permission_repository.read(config=config, params=params)
        if not result:
