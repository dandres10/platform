...
from src.domain.models.entities.rol.index import Rol, RolUpdate
...
class RolUpdateUseCase:
    ...
    async def execute(
        ...
        result = await self.rol_repository.update(config=config, params=params)
        if not result:
