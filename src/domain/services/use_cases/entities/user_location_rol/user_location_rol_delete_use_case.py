...
from src.domain.models.entities.user_location_rol.index import UserLocationRolDelete, UserLocationRol
from src.domain.services.repositories.entities.i_user_location_rol_repository import (
    IUserLocationRolRepository,
)


class UserLocationRolDeleteUseCase:
    ...
    async def execute(
        ...
        result = await self.user_location_rol_repository.delete(config=config, params=params)
        if not result:
