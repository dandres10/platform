...
from src.domain.models.entities.user_location_rol.index import UserLocationRol, UserLocationRolUpdate
...
class UserLocationRolUpdateUseCase:
    ...
    async def execute(
        ...
        result = await self.user_location_rol_repository.update(config=config, params=params)
        if not result:
