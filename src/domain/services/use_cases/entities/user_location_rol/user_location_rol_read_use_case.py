...
from src.domain.models.entities.user_location_rol.index import UserLocationRol, UserLocationRolRead
from src.domain.services.repositories.entities.i_user_location_rol_repository import (
    IUserLocationRolRepository,
)


class UserLocationRolReadUseCase:
    ...
    async def execute(
        ...
        result = await self.user_location_rol_repository.read(config=config, params=params)
        if not result:
