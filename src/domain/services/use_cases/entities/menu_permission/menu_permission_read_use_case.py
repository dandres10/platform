...
from src.domain.models.entities.menu_permission.index import MenuPermission, MenuPermissionRead
from src.domain.services.repositories.entities.i_menu_permission_repository import (
    IMenuPermissionRepository,
)


class MenuPermissionReadUseCase:
    ...
    async def execute(
        ...
        result = await self.menu_permission_repository.read(config=config, params=params)
        if not result:
