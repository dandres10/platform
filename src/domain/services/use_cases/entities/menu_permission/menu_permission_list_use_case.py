...
from src.domain.models.entities.menu_permission.index import MenuPermission
...
class MenuPermissionListUseCase:
    ...
    async def execute(
        ...
        config: Config,
        params: Pagination,
    ) -> Union[List[MenuPermission], str, None]:
        results = await self.menu_permission_repository.list(config=config, params=params)
        if not results:
