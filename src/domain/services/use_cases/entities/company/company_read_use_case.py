...
from src.domain.models.entities.company.index import Company, CompanyRead
from src.domain.services.repositories.entities.i_company_repository import (
    ICompanyRepository,
)


class CompanyReadUseCase:
    ...
    async def execute(
        ...
        result = await self.company_repository.read(config=config, params=params)
        if not result:
