...
from src.domain.models.entities.country.index import Country
...
class CountryListUseCase:
    ...
    async def execute(
        ...
        config: Config,
        params: Pagination,
    ) -> Union[List[Country], str, None]:
        results = await self.country_repository.list(config=config, params=params)
        if not results:
