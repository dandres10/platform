...
from src.domain.models.entities.country.index import Country, CountryRead
from src.domain.services.repositories.entities.i_country_repository import (
    ICountryRepository,
)


class CountryReadUseCase:
    ...
    async def execute(
        ...
        result = await self.country_repository.read(config=config, params=params)
        if not result:
