...
from src.domain.models.entities.user_country.index import UserCountry, UserCountryDelete
from src.domain.services.repositories.entities.i_user_country_repository import (
    IUserCountryRepository,
)


class UserCountryDeleteUseCase:
    ...
    async def execute(
        ...
        result = await self.user_country_repository.delete(config=config, params=params)
        if not result:
            return await self.message.get_message(
                config=config,
                message=MessageCoreEntity(
                    key=KEYS_MESSAGES.CORE_ERROR_DELETING_RECORD.value
                ),
            )
