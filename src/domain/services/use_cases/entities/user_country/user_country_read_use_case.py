...
from src.domain.models.entities.user_country.index import UserCountry, UserCountryRead
from src.domain.services.repositories.entities.i_user_country_repository import (
    IUserCountryRepository,
)


class UserCountryReadUseCase:
    ...
    async def execute(
        ...
        result = await self.user_country_repository.read(config=config, params=params)
        if not result:
