...
from src.domain.models.entities.user_country.index import UserCountry, UserCountryUpdate
from src.domain.services.repositories.entities.i_user_country_repository import (
    IUserCountryRepository,
)


class UserCountryUpdateUseCase:
    ...
    async def execute(
        ...
        result = await self.user_country_repository.update(config=config, params=params)
        if not result:
            return await self.message.get_message(
                config=config,
                message=MessageCoreEntity(
                    key=KEYS_MESSAGES.CORE_ERROR_UPDATING_RECORD.value
                ),
            )
