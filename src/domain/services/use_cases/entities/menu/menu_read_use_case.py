...
from src.domain.models.entities.menu.index import Menu, MenuRead
from src.domain.services.repositories.entities.i_menu_repository import (
    IMenuRepository,
)


class MenuReadUseCase:
    ...
    async def execute(
        ...
        result = await self.menu_repository.read(config=config, params=params)
        if not result:
