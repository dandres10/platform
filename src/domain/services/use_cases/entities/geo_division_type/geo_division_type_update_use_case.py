...
from src.domain.models.entities.geo_division_type.index import GeoDivisionType, GeoDivisionTypeUpdate
...
class GeoDivisionTypeUpdateUseCase:
    ...
    async def execute(
        ...
        result = await self.geo_division_type_repository.update(config=config, params=params)
        if not result:
