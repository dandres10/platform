...
from src.domain.models.entities.geo_division_type.index import GeoDivisionType, GeoDivisionTypeRead
from src.domain.services.repositories.entities.i_geo_division_type_repository import (
    IGeoDivisionTypeRepository,
)


class GeoDivisionTypeReadUseCase:
    ...
    async def execute(
        ...
        result = await self.geo_division_type_repository.read(config=config, params=params)
        if not result:
