...
from src.domain.models.entities.geo_division_type.index import GeoDivisionTypeDelete, GeoDivisionType
from src.domain.services.repositories.entities.i_geo_division_type_repository import (
    IGeoDivisionTypeRepository,
)


class GeoDivisionTypeDeleteUseCase:
    ...
    async def execute(
        ...
        result = await self.geo_division_type_repository.delete(config=config, params=params)
        if not result:
