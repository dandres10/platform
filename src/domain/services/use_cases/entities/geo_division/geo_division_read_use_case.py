...
from src.domain.models.entities.geo_division.index import GeoDivision, GeoDivisionRead
from src.domain.services.repositories.entities.i_geo_division_repository import (
    IGeoDivisionRepository,
)


class GeoDivisionReadUseCase:
    ...
    async def execute(
        ...
        result = await self.geo_division_repository.read(config=config, params=params)
        if not result:
