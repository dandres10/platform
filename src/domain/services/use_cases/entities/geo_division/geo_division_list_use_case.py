...
from src.domain.models.entities.geo_division.index import GeoDivision
...
class GeoDivisionListUseCase:
    ...
    async def execute(
        ...
        config: Config,
        params: Pagination,
    ) -> Union[List[GeoDivision], str, None]:
        results = await self.geo_division_repository.list(config=config, params=params)
        if not results:
