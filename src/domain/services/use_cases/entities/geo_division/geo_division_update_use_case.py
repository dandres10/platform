...
from src.domain.models.entities.geo_division.index import GeoDivision, GeoDivisionUpdate
...
class GeoDivisionUpdateUseCase:
    # SPEC-004 D6
    def __init__(
        self,
    ...
    async def execute(
        ...
        result = await self.geo_division_repository.update(config=config, params=params)
        if not result:
