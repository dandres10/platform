...
from src.domain.models.entities.geo_division.index import GeoDivisionDelete, GeoDivision
from src.domain.services.repositories.entities.i_geo_division_repository import (
    IGeoDivisionRepository,
)


class GeoDivisionDeleteUseCase:
    ...
    async def execute(
        ...
        result = await self.geo_division_repository.delete(config=config, params=params)
        if not result:
