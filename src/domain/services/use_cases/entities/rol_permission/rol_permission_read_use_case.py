...
from src.domain.models.entities.rol_permission.index import RolPermission, RolPermissionRead
from src.domain.services.repositories.entities.i_rol_permission_repository import (
    IRolPermissionRepository,
)


class RolPermissionReadUseCase:
    ...
    async def execute(
        ...
        result = await self.rol_permission_repository.read(config=config, params=params)
        if not result:
