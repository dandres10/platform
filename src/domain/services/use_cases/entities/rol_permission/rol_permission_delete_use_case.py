...
from src.domain.models.entities.rol_permission.index import RolPermissionDelete, RolPermission
from src.domain.services.repositories.entities.i_rol_permission_repository import (
    IRolPermissionRepository,
)


class RolPermissionDeleteUseCase:
    ...
    async def execute(
        ...
        result = await self.rol_permission_repository.delete(config=config, params=params)
        if not result:
