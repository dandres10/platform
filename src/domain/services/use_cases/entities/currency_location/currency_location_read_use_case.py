...
from src.domain.models.entities.currency_location.index import CurrencyLocation, CurrencyLocationRead
from src.domain.services.repositories.entities.i_currency_location_repository import (
    ICurrencyLocationRepository,
)


class CurrencyLocationReadUseCase:
    ...
    async def execute(
        ...
        result = await self.currency_location_repository.read(config=config, params=params)
        if not result:
