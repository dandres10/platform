...
from src.domain.models.entities.currency_location.index import CurrencyLocation
...
class CurrencyLocationListUseCase:
    ...
    async def execute(
        ...
        config: Config,
        params: Pagination,
    ) -> Union[List[CurrencyLocation], str, None]:
        results = await self.currency_location_repository.list(config=config, params=params)
        if not results:
