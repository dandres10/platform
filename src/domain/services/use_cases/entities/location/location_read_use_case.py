...
from src.domain.models.entities.location.index import Location, LocationRead
from src.domain.services.repositories.entities.i_location_repository import (
    ILocationRepository,
)


class LocationReadUseCase:
    ...
    async def execute(
        ...
        result = await self.location_repository.read(config=config, params=params)
        if not result:
